...
def cat_feature_engineer(X, encode_type='count'):
    ...
    if encode_type == 'count':
        return CountFrequencyEncoder(variables=cat_features, encoding_method='count').fit_transform(X)
    elif encode_type == 'onehot':
        return OneHotEncoder(variables=cat_features, drop_last=True, drop_last_binary=True).fit_transform(X)
    else:
...
def num_feature_engineer(X, switch=True):
    ...
    if switch:
        ...
        num_agg = X.groupby('age')[num_features].agg(['mean', 'std', 'min', 'max', 'last'])
