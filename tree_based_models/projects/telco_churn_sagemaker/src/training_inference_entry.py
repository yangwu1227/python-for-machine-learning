...
def parser():
    ...
    parser.add_argument('--s3_bucket', '-s3_bucket', type=str, default='yang-ml-sagemaker')
...
def input_fn(input_data: bytes, content_type: str) -> pd.DataFrame:
    ...
    # Feature names
    feature_names = ['gender', 'age', 'under_30', 'senior_citizen', 'married',
                     'dependents', 'number_of_dependents', 'number_of_referrals',
    ...
    # Feature types
    dtype_dict = {'gender': 'category', 'age': 'int64', 'under_30': 'category', 'senior_citizen': 'category', 'married': 'category', 
                                  'dependents': 'category', 'number_of_dependents': 'int64', 'number_of_referrals': 'int64', 'tenure_in_months': 'int64', 
                                  'offer': 'category', 'phone_service': 'category', 'avg_monthly_long_distance_charges': 'float64', 'multiple_lines': 'category',
                                  'internet_service': 'category', 'internet_type': 'category', 'avg_monthly_gb_download': 'int64', 'online_security': 'category',
                                  'online_backup': 'category', 'device_protection_plan': 'category', 'premium_tech_support': 'category', 'streaming_tv': 'category',
                                  'streaming_movies': 'category', 'streaming_music': 'category', 'unlimited_data': 'category', 'contract': 'category', 
                                  'paperless_billing': 'category', 'payment_method': 'category', 'monthly_charge': 'float64', 'total_charges': 'float64', 
                                  'total_refunds': 'float64', 'total_extra_data_charges': 'int64', 'total_long_distance_charges': 'float64', 
                                  'satisfaction_score': 'int64', 'cltv': 'int64'}
    
    # Check that the input dataframe has the correct number of columns
    if len(data.columns) != len(feature_names):
        raise AttributeError('The input data does not have the right number of features')
...
if __name__ == '__main__':
    ...
    X_train, y_train = load_data(
        data_s3_url=os.path.join(args.train, 'train.csv'),
        logger=None
    )
    ...
    model_pipeline = create_pipeline(num_feat=hyperparams['num_feat'], step=hyperparams['step'])
