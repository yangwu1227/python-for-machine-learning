...
from custom_utils import (
    ...
    create_study, study_report, create_preprocessor
)
from inference import model_fn, input_fn, predict_fn

# ------------------ Function for creating xgboost estimator ----------------- #

def create_xgb_estimator(hyperparameters: Dict[str, Any], validation: bool, scale_pos_weight: float) -> xgb.XGBClassifier:
    """
...
def xgboost_objective(trial: optuna.Trial, 
    ...
    model_hyperparameter = {
        ...
        'min_child_weight': trial.suggest_float('min_child_weight', 0, 20), # Smaller values since this is a imbalanced dataset and small number of samples from a minority class can be in a leaf
...
if __name__ == '__main__':
    ...
    best_model = joblib.load(os.path.join('/tmp', f'model_trial_{study.best_trial.number}.joblib'))
