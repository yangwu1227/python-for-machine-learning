...
class FeatureEngine(TransformerMixin, BaseEstimator):
    ...
    def __init__(
        self, top_5_feat: List[str], top_15_feat: List[str], cat_feat: List[str]
    ):
        """
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for group in self.cat_feat:
            for agg_func in ["mean", "max", "sum"]:
                X[[col + f"_{agg_func}_by_{group}" for col in self.top_5_feat]] = (
                    X.groupby(group)[self.top_5_feat].transform(agg_func)
                )
        ...
        pairwise_cols = [
            f"{col_i}_{func_key}_{col_j}"
            for func_key in py_operators
            for col_i, col_j in col_pairs
        ]
        triplet_cols = [
            f"{col_q}_{func_key}_{col_t}_{func_key}_{col_k}"
            for func_key in py_operators
        ...
        pairwise_interactions = pd.DataFrame(
            index=X.index, columns=pairwise_cols, dtype=float
        )
        triplet_interactions = pd.DataFrame(
            index=X.index, columns=triplet_cols, dtype=float
        )

        # Pairwise interactions for the top 5 most important features
        for func_key in py_operators:
            for col_i, col_j in col_pairs:
                pairwise_interactions[f"{col_i}_{func_key}_{col_j}"] = py_operators[
                    func_key
                ](X[col_i], X[col_j])
        ...
        for func_key in py_operators:
            for col_q, col_t, col_k in col_triplets:
                triplet_interactions[
                    f"{col_q}_{func_key}_{col_t}_{func_key}_{col_k}"
                ] = py_operators[func_key](
                    X[col_q], py_operators[func_key](X[col_t], X[col_k])
                )
...
class FeatureImportanceHandler:
    ...
    def __init__(
        self, s3_key: str, s3_bucket: str, job_name: str, trial_number: int
    ) -> None:
        """
        Parameters
        ----------
    ...
    def impure_feat_imp(model_pipeline: Pipeline) -> Dict[str, float]:
        ...
        """
        feature_names = model_pipeline.named_steps["rf_clf"].feature_names_in_
        impurity_imp_dict = dict(
            zip(
                feature_names, model_pipeline.named_steps["rf_clf"].feature_importances_
            )
        )
