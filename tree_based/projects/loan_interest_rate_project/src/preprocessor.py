...
def extract_date_features(data: pd.DataFrame) -> cudf.DataFrame:
    ...
    # Year extraction
    data[year_cols] = data[date_cols].apply(lambda col: pd.to_datetime(
        col, errors='coerce').dt.to_period("M").dt.year)
    data[month_cols] = data[date_cols].apply(lambda col: pd.to_datetime(
        col, errors='coerce').dt.to_period("M").dt.month)
...
def num_feat_eng(data: cudf.DataFrame) -> cudf.DataFrame:
    ...
    num_cols = ['loan_amt_requested', 'loan_amt_investor_funded_portion', 'borrower_annual_income', 'monthly_debt_to_income_ratio',
                'num_of_past_dues', 'num_of_creditor_inquiries', 'num_of_open_credit_line', 'num_of_derog_publib_rec', 'total_credit_rev_balance',
                'rev_line_util_rate', 'total_credit_line']

    group_cols = ['num_of_payment_months', 'loan_subgrade', 'num_of_years_employed', 'home_ownership_status', 'verify_income_or_source',
                  'loan_issued_year', 'borrower_provided_loan_category', 'zip_first_three', 'borrower_state', 'borrower_earliest_credit_open_year',
                  'init_loan_status']

    for group in group_cols:
        for agg_func in ['std', 'mean', 'max', 'sum']:
            data[[col + f'_{agg_func}_by_{group}' for col in num_cols]
                 ] = data.groupby(group)[num_cols].transform(agg_func)
