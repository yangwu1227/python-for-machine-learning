...
def train_baseline(model_func: Callable, hyperparameters: Dict[str, Any], train_data: List[Union[pd.DataFrame, pd.Series]], logger: logging.Logger) -> Dict[str, Any]:    
    """
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(train_data[0], train_data[1])):
        ...
        X_train, X_val = cudf.DataFrame.from_pandas(train_data[0].iloc[train_index]), cudf.DataFrame.from_pandas(train_data[0].iloc[val_index])
        y_train, y_val = cudf.Series.from_pandas(train_data[1].iloc[train_index]), cudf.Series.from_pandas(train_data[1].iloc[val_index])

        # Compute sample weights (using numpy)
        fold_sample_weights = compute_sample_weight(class_weight='balanced', y=y_train.values.get())
        ...
        logger.info(f'Extracting best score and feature importance for fold {fold + 1}...')
        logloss_scores[f'fold_{fold + 1}'] = np.max(fold_pipeline.named_steps['xgb_clf'].evals_result()['validation_1']['mlogloss'])
        # Feature importance must be obtained from the booster object
        feature_importances[f'fold_{fold + 1}'] = fold_pipeline['xgb_clf'].get_booster().get_score(importance_type='total_gain')
