...
def create_lgb_estimator(hyperparameters: Dict[str, Any]) -> Tuple[lgb.LGBMClassifier, Union[None, Callable]]:
    """
...
def train_lgb(pipeline_func: Callable, 
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(train_data[0], train_data[1])):
        ...
        # Fit model
        fold_pipeline = pipeline_func(inter_feat, poly_feat, hyperparameters['svd'], hyperparameters['n_components'])

        logger.info(f'Preprocessing training and validation data for fold {fold + 1}...')
        ...
        fold_estimator, fold_lgb_early_stopping = estimator_func(hyperparameters=hyperparameters)
        ...
        logloss_scores[f'fold_{fold + 1}'] = fold_estimator.best_score_['val']['multi_logloss']
    ...
    model_pipeline = pipeline_func(inter_feat, poly_feat, hyperparameters['svd'], hyperparameters['n_components'])
    ...
    lgb_estimator.fit(X=X_train, y=y_train, sample_weight=sample_weights, eval_set=[(X_train, y_train), (X_train, y_train)], callbacks=[lgb_early_stopping])
...
if __name__ == '__main__':
    ...
    poly_feat = inter_feat + ['feat_14', 'feat_15', 'feat_36', 'feat_39', 'feat_40', 'feat_50', 'feat_62', 'feat_75', 'feat_86', 'feat_90']
