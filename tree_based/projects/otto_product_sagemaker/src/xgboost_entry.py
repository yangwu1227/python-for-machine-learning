...
def create_xgb_estimator(hyperparameters: Dict[str, Any], validation: bool) -> xgb.XGBClassifier:
    """
...
def train_xgboost(pipeline_func: Callable, 
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(train_data[0], train_data[1])):
        ...
        # Create estimator
        fold_estimator = estimator_func(hyperparameters=hyperparameters, validation=True)
        ...
        logloss_scores[f'fold_{fold + 1}'] = np.min(fold_estimator.evals_result()['validation_1']['mlogloss'])
    ...
    xgb_estimator.fit(X=X_train, y=y_train, sample_weight=sample_weights, verbose=200, eval_set=[(X_train, y_train)])
