...
def gen_fraud_data(num_unique_ccs: int = NUM_UNIQUE_CCS, start_trans_date: datetime.datetime = START_TRANS_DATE, end_trans_date: datetime.datetime = END_TRANS_DATE) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    fake = Faker()
    cc_nums = [fake.credit_card_number() for _ in range(
        num_unique_ccs)]  # Generate a valid and unique credit card numbers

    cc_types = [fake.credit_card_provider() for _ in range(
        num_unique_ccs)]  # Generate credit card provider names

    num_trans_per_cc = np.ceil(np.random.exponential(
        scale=3, size=num_unique_ccs)).astype(np.int32)  # Generate numbers of transactions per credit card number, the overall distribution follows an exponential distribution with most credit card numbers having fewer transactions

    # Generate random IPv4 addresses or network with a valid CIDR
    cc_ipv4 = [fake.ipv4() for _ in range(num_unique_ccs)]

    cc_phone_number = [fake.phone_number()for _ in range(
        num_unique_ccs)]  # Generate random phone numbers
    ...
    data = {
        ...
        'TransactionDT': [fake.date_time_between_dates(datetime_start=start_trans_date, datetime_end=end_trans_date)
                          for _ in range(sum(num_trans_per_cc))],  # Generate random dates and times between two given dates
    ...
    identity_transactions_idx = np.random.choice(a=transactions.shape[0], size=int(
        transactions.shape[0]*0.8), replace=False)  # Generate random indices for the identity table
    ...
    full_two_df = transactions[["TransactionID", "card_no", "card_type", "email_domain",
                                "ProductCD", "TransactionAmt"]].merge(identity, on='TransactionID', how='left')
    ...
    for idx, row in full_two_df.iterrows():
        ...
        card_no, card_type, email, product_type, transcation_amount, ip_address, phone_no, device_id = str(
            row["card_no"]), row["card_type"], row["email_domain"], row["ProductCD"], row["TransactionAmt"], str(row["IpAddress"]), str(row["PhoneNo"]), str(row["DeviceID"])
        ...
        if email in ["hotmail.com", "gmail.com", "yahoo.com"]:
            ...
            if transcation_amount > 3000:
                ...
                if card_type in ["Diners Club / Carte Blanche", "JCB 15 digit", "Maestro"]:
