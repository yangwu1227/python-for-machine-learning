"""
Logistic loss plotting routines and support
"""
...
def compute_cost_logistic_sq_err(X, y, w, b):
    """
    Compute sq error cost on logistic data (for negative example only, not used in practice).

    Parameters
    ----------
    X : ndarray
        Shape (m,n) matrix of examples with multiple features.
    w : ndarray
        Shape (n) parameters for prediction.
    b : scalar
...
def plt_logistic_squared_error(X, y):
    ...
    # Unravel returns a contiguous flattened array like reshape(-1, order=order)
    points = np.c_[wx.ravel(), by.ravel()]
    cost = np.zeros(points.shape[0])

    for i in range(points.shape[0]):
        w, b = points[i]
        # Flattens X to 1d-array
