...
class plt_quad_logistic:
    ...
    dplot : DataPlot
        Data plot object.
    con_plot : ContourAndSurfacePlot
        Contour and surface plot object.
    cplot : CostPlot
        Cost plot object.
    cid : int
        Connection id for the event.
    bcalc : matplotlib.widgets.Button
        Button for running gradient descent.
    """
    
    def __init__(self, x_train, y_train, w_range, b_range):
        """
        Initialize the PltQuadLogistic object.
...
class data_plot:
    ...
    b : float
        Bias.
    aline : list
        Line object for the logistic line.
    bline : list
        Line object for the z line.
    alegend : matplotlib.legend.Legend
        Legend for the plot.
    cost_items : list
        List of cost items.
    """
    ...
    def __init__(self, ax, x_train, y_train, w, b):
        """
        Initialize the DataPlot object.
    ...
    def draw_logistic_lines(self, firsttime=False):
        ...
        firsttime : bool, optional
            Whether it is the first time drawing the lines (default is False).
        """
        if not firsttime:
            self.aline[0].remove()
            self.bline[0].remove()
    ...
    def mk_cost_lines(self, firsttime=False):
        ...
        ----------
        firsttime : bool, optional
            Whether it is the first time making the lines (default is False).
        """
        if not firsttime:
...
class contour_and_surface_plot:
    ...
    axc : matplotlib.axes.Axes
        The axes object for the contour plot.
    axs : matplotlib.axes.Axes
        The axes object for the surface plot.
    path : Path
        Path object for tracking paths during gradient descent.
    dyn_items : list
    ...
    def __init__(self, axc, axs, x_train, y_train, w_range, b_range, w, b):
        """
        Initialize the ContourAndSurfacePlot object.
        ...
        self.path = path(self.w, self.b, self.axc)  # Initialize an empty path, avoids existence check

    def update_contour_wb_lines(self, w, b, firsttime=False):
        """
        Update contour w, b lines.
        ...
        b : float
            Bias.
        firsttime : bool, optional
            Whether it is the first time updating the lines (default is False).
        """
        self.w = w
        self.b = b
...
class cost_plot:
    ...
    Attributes:
        ...
        costs: A list to store the cost values.
        cline: The line object for the cost plot.
    """
    def __init__(self, ax):
        """Initializes the CostPlot with the given axis.
    ...
    def re_init(self):
        """Re-initializes the CostPlot by clearing the axis and re-initializing."""
        self.ax.clear()
        self.__init__(self.ax)

    def add_cost(self, J_hist):
        """Adds the cost values to the plot.

        Args:
            J_hist: The cost values to add.
        """
...
class path:
    ...
    Attributes:
        path_items: A list to store the path items.
        w: The w value at the start of the path.
        b: The b value at the start of the path.
        ax: The axis for the plot.
    """
    def __init__(self, w, b, ax):
        """Initializes the Path with the given w, b, and axis.
    ...
    def re_init(self, w, b):
        """Re-initializes the Path with the given w and b.
    ...
    def add_path_item(self, w, b):
        """Adds a new path item to the plot.
...
def truncate_colormap(cmap, minval=0.0, maxval=1.0, n=100):
    ...
    minval: The minimum value to truncate.
    maxval: The maximum value to truncate.
    n: The number of values to truncate to.

    Returns:
        new_cmap: The truncated color map.
    """
