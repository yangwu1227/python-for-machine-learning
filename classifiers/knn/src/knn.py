...
class KNNClassifier(BaseEstimator, ClassifierMixin):
    ...
    """
    def __init__(self, n_neighbors: int = 5, weights: str = 'distance', metric: str = 'cosine', metric_params: Dict[str, Any] = None) -> None:
        """
    ...
    def fit(self, X: Union[csr_matrix, np.ndarray, pd.DataFrame], y: Union[np.ndarray, pd.Series, List[Union[str, float]]]) -> "KNNClassifier":
        """
        ...
        None
        """
        X, y = check_X_y(X=X, y=y, accept_sparse='csr', ensure_2d=True, allow_nd=False, y_numeric=False)
    ...
    def predict(self, X: Union[csr_matrix, np.ndarray, pd.DataFrame]) -> np.ndarray:
        ...
        """
        check_is_fitted(self, ['X_train_', 'y_train_', 'label_encoder_', 'classes_', 'n_features_in_'])
    ...
    def _compute_distances(self, test_sample: Union[np.ndarray, csr_matrix]) -> np.ndarray:
        """
        ...
        if self.metric == 'cosine':
            return 1 - cosine_similarity(X=test_sample, Y=self.X_train_, **metric_params).flatten()
        elif self.metric == 'euclidean':
            return euclidean_distances(X=test_sample, Y=self.X_train_, **metric_params).flatten()
        else:
            raise NotImplementedError(f"Unsupported similarity function '{self.metric}'")

    def _predict_from_neighbors(self, k_indices: np.ndarray, distances: np.ndarray) -> int:
        """
        ...
        if self.weights == 'distance':
            ...
            weighted_votes = np.bincount(k_nearest_labels, weights=inv_distances, minlength=len(self.classes_))
