...
class RestApiManager(object):
    ...
    def _create_resource(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Created resource {self.api_base_path} under root resource with ID {self.resource_id}"
            )
        except ClientError as error:
            self.logger.exception(
    ...
    def _setup_lambda_integration(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Set up Lambda integration for POST method on resource {self.resource_id}"
            )
        except ClientError as error:
            self.logger.exception(
    ...
    def _deploy_rest_api(self) -> None:
        ...
        try:
            self.apigateway_client.create_deployment(
                restApiId=self.api_id, stageName=self.api_stage
            )
    ...
    def _grant_permission(self) -> None:
        ...
        None
        """
        source_arn = f"arn:aws:execute-api:{self.apigateway_client.meta.region_name}:{self.account_id}:{self.api_id}/*/POST/{self.api_base_path}"
        try:
            ...
            self.logger.info(
                f"Granted permission to let Amazon API Gateway invoke function {self.lambda_function_arn} from {source_arn}"
            )
        except ClientError as error:
            self.logger.exception(
    ...
    def _create_api_key(self, api_key_name: str, enabled: bool) -> None:
        ...
        try:
            response = self.apigateway_client.create_api_key(
                name=api_key_name, enabled=enabled
            )
    ...
    def _create_usage_plan(self, usage_plan_name: str) -> None:
        ...
        try:
            response = self.apigateway_client.create_usage_plan(
                ...
                apiStages=[{"apiId": self.api_id, "stage": self.api_stage}],
    ...
    def _add_api_key_to_usage_plan(self) -> None:
        ...
        try:
            self.apigateway_client.create_usage_plan_key(
                usagePlanId=self.usage_plan_id, keyId=self.api_key_id, keyType="API_KEY"
            )
            self.logger.info(
                f"Added API key {self.api_key_id} to usage plan {self.usage_plan_id}"
            )
        except ClientError as error:
            self.logger.exception(
    ...
    def _rollback_created_resources(self) -> None:
        ...
        try:
            self.lambda_client.remove_permission(
                FunctionName=self.lambda_function_arn, StatementId="InvokeByApiGateway"
            )
            self.logger.info(
                f'Removed permission "InvokeByApiGateway" from function {self.lambda_function_arn}'
            )
        except ClientError as error:
            if error.response["Error"]["Code"] == "ResourceNotFoundException":
                # If the permission or the lambda function does not exist, then there is nothing to do
                self.logger.info(
                    "Nothing to remove as the specified Lambda function does not exist"
                )
            else:
                self.logger.error(
        ...
        if self.usage_plan_id and self.api_key_id:
            try:
                self.apigateway_client.delete_usage_plan_key(
                    usagePlanId=self.usage_plan_id, keyId=self.api_key_id
                )
                self.logger.info(
                    f"Deleted API key {self.api_key_id} from usage plan {self.usage_plan_id}"
                )
            except ClientError as error:
        ...
        if self.resource_id:
            try:
                self.apigateway_client.delete_resource(
                    restApiId=self.api_id, resourceId=self.resource_id
                )
        ...
        try:
            ...
            if apiKeyRequired:
                if not api_key_name or not enabled or not usage_plan_name:
                    raise ValueError(
                        "API key name, enabled, and usage plan name must be specified"
                    )
    ...
    def invoke_rest_api(self, payload: Dict[str, str]) -> Dict[str, str]:
        ...
        if self._api_key_value:
            ...
            try:
                ...
                self.logger.info(
                    f"Invoked REST API {self.api_id} with payload {payload} and API key {self.api_key_id}"
                )
                return response.json()
            except requests.exceptions.RequestException as error:
                self.logger.exception(
                    f"Cannot invoke REST API {self.api_id} due to {error}"
                )
                raise
        else:
            # No API key required
            try:
                ...
                self.logger.info(
                    f"Invoked REST API {self.api_id} with payload {payload}"
                )
                return response.json()
            except requests.exceptions.RequestException as error:
                self.logger.exception(
