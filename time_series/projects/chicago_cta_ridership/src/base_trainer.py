...
class BaseTrainer(object):
    ...
    """
    def __init__(self, horizon: str, config_path: str, logger_name: str, config_name: str, s3_helper: S3Helper = None) -> None:
        """
        ...
        self.logger, self.config = SetUp(logger_name=logger_name, config_name=config_name, config_path=config_path).setup()
    ...
    def _process_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        return data.astype(np.float32) # Should be float32 for sktime or else it will fail
    ...
    def load_and_process_data(self) -> Dict[str, pd.DataFrame]:
        ...
        for data_name in ['train', 'test']:
            s3_obj_key = os.path.join(self.config[self.horizon]['output_key'], f'{data_name}.parquet')
    ...
    def cross_validate(self):
        ...
        """
        raise NotImplementedError('Must implement cross_validation() method in child class')
    ...
    def refit_and_forecast(self, level: float = 0.95) -> Dict[str, Union[pd.DataFrame, Dict[str, pd.DataFrame]]]:
        ...
        Dict[str, Union[pd.DataFrame, Dict[str, pd.DataFrame]]]
            ...
            intervals for each target.
        """
        raise NotImplementedError('Must implement refit_and_forecast() method in child class')
    ...
    @staticmethod
    def extract_prediction_intervals(pi: pd.DataFrame, level: float) -> Dict[str, pd.DataFrame]:
        """
    ...
    @staticmethod
    def _diagnostic_tests(residuals: pd.DataFrame, lags: int = None, auto_lag: bool = None) -> Dict[str, pd.DataFrame]:
        """
        ...
        for target in residuals.columns:
            ...
            sw_result = ['Shapiro-Wilk', 'Normal', sw_p_value, sw_p_value < 0.01, sw_p_value < 0.05, sw_p_value < 0.10]
            
            # Ljung-Box Test
            lb_stat, lb_p_value = acorr_ljungbox(series, lags=lags, return_df=True, auto_lag=auto_lag).values[0]
            lb_result = ['Ljung-Box', 'No Autocorrelation', lb_p_value, lb_p_value < 0.01, lb_p_value < 0.05, lb_p_value < 0.10]
            
            tests_results = pd.DataFrame([sw_result, lb_result], columns=['Test', 'Null Hypothesis', 'P-Value', 'Reject at 1%', 'Reject at 5%', 'Reject at 10%'])
            results[target] = tests_results
            
        return results
        
    @staticmethod
    def plot_forecast(start_date: str, 
                      ...
                      title: str = None,
                      height: int = 450,
                      width: int = 1200) -> Union[None, Image]:
        """
        Plot the forecast.
    ...
    @staticmethod
    def plot_forecast_comparison(start_date: str, 
...
class ForecastVisualizer(object):
    ...
    """
    @staticmethod
    def _prepare_data_for_plotting(start_date: str, **data_objs: Union[pd.DataFrame, Dict[str, pd.DataFrame]]) -> Dict[str, pd.DataFrame]:
        """
    ...
    @staticmethod
    def _update_layout_and_return_figure(fig: Figure, title: str, static: bool, height: int, width: int) -> Optional[Image]:
        """
    ...
    def plot_forecast(start_date: str, 
        ...
        y_train, y_test, y_pred, pi = data_dict['y_train'], data_dict['y_test'], data_dict['y_pred'], data_dict['pi']
        ...
        for i, var_name in enumerate(y_train.columns):
            ...
            # Training Data
            fig.add_trace(go.Scatter(x=y_train.index, y=y_train[var_name], mode='lines', name='Train', line=dict(color='blue'), showlegend=showlegend), row=1, col=i+1)
            
            # Prediction Intervals
            fig.add_trace(go.Scatter(x=pi[var_name].index, y=pi[var_name]['lower'], mode='lines', name='Lower PI', line=dict(width=0), showlegend=showlegend), row=1, col=i+1)
            fig.add_trace(go.Scatter(x=pi[var_name].index, y=pi[var_name]['upper'], mode='lines', name='Upper PI', line=dict(width=0), fill='tonexty', fillcolor='rgba(0,100,80,0.2)', showlegend=showlegend), row=1, col=i + 1)
            
            # Predictions
            fig.add_trace(go.Scatter(x=y_pred.index, y=y_pred[var_name], mode='lines', name='Forecast', line=dict(color='orange'), showlegend=showlegend), row=1, col=i+1)
            
            # Test Data (if provided)
            if y_test is not None and var_name in y_test:
                fig.add_trace(go.Scatter(x=y_test.index, y=y_test[var_name], mode='lines', name='Test', line=dict(color='green'), showlegend=showlegend), row=1, col=i+1)
                
            y_axis_title = 'Bus Ridership' if var_name == 'bus' else 'Rail Boardings'
            fig.update_yaxes(title_text=y_axis_title, row=1, col=i + 1)
        
        return ForecastVisualizer._update_layout_and_return_figure(fig=fig, title=title, static=static, height=height, width=width)
    ...
    def plot_forecast_comparison(start_date: str, 
        ...
        y_train_original, y_pred_original = data_dict['y_train_original'], data_dict['y_pred_original']
        y_train_counterfactual, y_pred_counterfactual = data_dict['y_train_counterfactual'], data_dict['y_pred_counterfactual']
        pi_original, pi_counterfactual = data_dict['pi_original'], data_dict['pi_counterfactual']
        ...
        for i, var_name in enumerate(y_train_original.columns):
            ...
            # Training Data
            fig.add_trace(go.Scatter(x=y_train_original.index, y=y_train_original[var_name], mode='lines', name='Train (Original)', line=dict(color='blue'), showlegend=showlegend), row=1, col=i+1)
            fig.add_trace(go.Scatter(x=y_train_counterfactual.index, y=y_train_counterfactual[var_name], mode='lines', name='Train (Counterfactual)', line=dict(color='pink'), showlegend=showlegend), row=1, col=i+1)
            
            # Prediction Intervals
            fig.add_trace(go.Scatter(x=pi_original[var_name].index, y=pi_original[var_name]['lower'], mode='lines', name='Lower PI (Original)', line=dict(width=0), showlegend=showlegend), row=1, col=i+1)
            fig.add_trace(go.Scatter(x=pi_original[var_name].index, y=pi_original[var_name]['upper'], mode='lines', name='Upper PI (Original)', line=dict(width=0), fill='tonexty', fillcolor='rgba(0,100,80,0.2)', showlegend=showlegend), row=1, col=i + 1)
            fig.add_trace(go.Scatter(x=pi_counterfactual[var_name].index, y=pi_counterfactual[var_name]['lower'], mode='lines', name='Lower PI (Counterfactual)', line=dict(width=0), showlegend=showlegend), row=1, col=i+1)
            fig.add_trace(go.Scatter(x=pi_counterfactual[var_name].index, y=pi_counterfactual[var_name]['upper'], mode='lines', name='Upper PI (Counterfactual)', line=dict(width=0), fill='tonexty', fillcolor='rgba(0,100,80,0.2)', showlegend=showlegend), row=1, col=i + 1)
            
            # Predictions
            fig.add_trace(go.Scatter(x=y_pred_original.index, y=y_pred_original[var_name], mode='lines', name='Forecast (Original)', line=dict(color='orange'), showlegend=showlegend), row=1, col=i+1)
            fig.add_trace(go.Scatter(x=y_pred_counterfactual.index, y=y_pred_counterfactual[var_name], mode='lines', name='Forecast (Counterfactual)', line=dict(color='orange'), showlegend=showlegend), row=1, col=i+1)
