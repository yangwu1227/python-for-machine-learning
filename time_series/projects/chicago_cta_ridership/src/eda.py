...
class ExploratoryDataAnalyzer(object):
    ...
    @staticmethod
    def plot_correlations(series: pd.Series, lags: int, suptitle: str, fig_size: Tuple[int], zero_lag: bool = False) -> None:
        """
    ...
    @staticmethod
    def plot_ccf(x: Union[pd.Series, np.ndarray], y: Union[pd.Series, np.ndarray], maxlags: int, title: str, fig_size: Tuple[int], ci: float = 0.95) -> None:
        """
        ...
        ax.axhline(ci_lim, color='blue', linestyle='--', label=f'{ci*100}% Significance Level (White Noise)')
    ...
    @staticmethod
    def plot_granger_causality(data: pd.DataFrame, maxlags: int, fig_size: Tuple[int]) -> None:
        """
        ...
        ssr_ftest_pvals = np.array([gc_res[i + 1][0]['ssr_ftest'][1] for i in range(len(gc_res))])
        ssr_chi2test_pvals = np.array([gc_res[i + 1][0]['ssr_chi2test'][1] for i in range(len(gc_res))])
        lrtest_pvals = np.array([gc_res[i + 1][0]['lrtest'][1] for i in range(len(gc_res))])
        params_ftest_pvals = np.array([gc_res[i + 1][0]['params_ftest'][1] for i in range(len(gc_res))])
    ...
    def plot_correlations_within_year(self, selections: List[int], var: str, lags: int, suptitle: str, fig_size: Tuple[int], zero_lag: bool = False) -> None:
        """
    ...
    def plot_correlations_between_years(self, var: str, lags: int, agg_func: str, freq: str, suptitle: str, fig_size: Tuple[int], zero_lag: bool = False) -> None:
        """
        ...
        series = self.data[['service_date', var]].set_index('service_date').resample(rule=freq).agg(agg_func)
    ...
    def plot_series_between_years(self, var: str, agg_func: str, freq: str, title: str, fig_size: Tuple[int]) -> None:
        """
        ...
        fig, ax = plt.subplots(figsize=fig_size)
        ax.plot(series)
        ax.set_title(f'{title} (Aggregation = {agg_func.title()}, Resampling Freq = {freq.upper()})')
        plt.show()
        
    def plot_conditional_distribution_and_timeplot(self, selections: List[int], var: str, fig_size: Tuple[int]) -> None:
        """
        Display an interactive plot based on user selections using ipywidgets. The plot
    ...
    def plot_monthly_distribution_by_year(self, selections: List[int], fig_size: Tuple[int, int]) -> None:
        """
        Display an interactive plot based on user selections using ipywidgets. The plot
        ...
        """
        def display_plot(year: int) -> None:
            year_subset_bus = self.data[self.data['service_date'].dt.year == year]['bus']
            year_subset_rail = self.data[self.data['service_date'].dt.year == year]['rail_boardings']
            year_subset_service_date = self.data[self.data['service_date'].dt.year == year]['service_date']

            fig, axes = plt.subplots(2, 1, figsize=fig_size)

            boxplot(x=year_subset_service_date.dt.month, y=year_subset_bus.values, ax=axes[0])
            axes[0].set_title(f'Bus Boardings by Month ({year})')
            axes[0].set_xlabel('Month')
            axes[0].set_ylabel('Bus Boardings')

            boxplot(x=year_subset_service_date.dt.month, y=year_subset_rail.values, ax=axes[1])
