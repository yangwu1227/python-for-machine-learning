...
class Processor(object):
    ...
    def __init__(self, logger: logging.Logger, config: Dict[str, Any], s3_helper: S3Helper = None):
        """
    ...
    def _ingest_data(self, input_key: str) -> pd.DataFrame:
        ...
        data[self.config['date_col']] = pd.to_datetime(
            data[self.config['date_col']])
        ...
        data[self.config['predictor']] = data[self.config['predictor']].map(self.config['day_type_map']).astype(pd.Int8Dtype())
    ...
    def _resample(self, data: pd.DataFrame, freq: str) -> pd.DataFrame:
        ...
        data = data.resample(rule=freq, on=self.config['date_col']).median().astype(np.int32)
    ...
    def _create_covid_model(self) -> ColumnEnsembleForecaster:
        ...
        bagging_ets = BaggingForecaster(
            ...
            forecaster=StatsForecastAutoETS(
                season_length=self.config['long']['m'],
                model='ZZZ'
            )
        )
    ...
    def _forecast_covid(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        return forecast_data.rename(columns={'index': 'service_date'}) # Reset index as a column so downstream data splitting tasks can use the date column
    ...
    def short_horizon(self) -> None:
        ...
        self.logger.info(f'Filtering data from {start_date.strftime("%Y-%m-%d")} to {end_date.strftime("%Y-%m-%d")}')
        data_2023 = data.loc[
            (data[self.config['date_col']] >= start_date) & (data[self.config['date_col']] <= end_date)
        ]

        # Forecasting horizon
        self.logger.info(f'Forecasting horizon: {forecast_horizon}')
        test_period_indices = pd.Index(data_2023[self.config['date_col']][-forecast_horizon:])

        # Split into train and test
        self.logger.info('Splitting data into train and test sets')
        train_start, train_end = min(data_2023[self.config['date_col']]).strftime('%Y-%m-%d'), (test_period_indices[0] - pd.Timedelta(days=1)).strftime('%Y-%m-%d')
        test_start, test_end = test_period_indices[0].strftime('%Y-%m-%d'), max(data_2023[self.config['date_col']]).strftime('%Y-%m-%d')

        self.logger.info(f'Train period: {train_start} to {train_end}')
        self.logger.info(f'Test period: {test_start} to {test_end}')
        train = data_2023.loc[~data_2023[self.config['date_col']].isin(test_period_indices)]
        test = data_2023.loc[data_2023[self.config['date_col']].isin(test_period_indices)]

        # Write train and test to s3
        for data, name in zip([train, test], ['train', 'test']):
            self.s3_helper.to_parquet(
                data=data,
                obj_key=os.path.join(self.config['short']['output_key'], f'{name}.parquet')
            )
            self.logger.info(f'Successfully written {name} data to s3 for short-horizon forecasting')
    ...
    def long_horizon(self) -> None:
        ...
        # Forecast horizon
        self.logger.info(f'Forecasting horizon: {self.config["long"]["forecast_horizon"]} months ahead')
        ...
        test_period_indices = pd.Index(monthly_data[self.config['date_col']][-forecast_horizon:])
        
        train_start, train_end = self.config['long']['start_date'], (test_period_indices[0] - pd.DateOffset(months=1)).strftime('%Y-%m-%d')
        test_start, test_end = test_period_indices[0].strftime('%Y-%m-%d'), self.config['long']['end_date']
        self.logger.info(f'Train period: {train_start} to {train_end}')
        self.logger.info(f'Test period: {test_start} to {test_end}')
        ...
        monthly_data.columns = ['original_' + col if col != self.config['date_col'] else col for col in monthly_data.columns]
        counterfactual_data.columns = ['counterfactual_' + col if col != self.config['date_col'] else col for col in counterfactual_data.columns]
        ...
        self.logger.info('Splitting data into train and test sets for long-horizon forecasting')

        # For long horizon (36 months), we start on 2013-01-31 and end on 2023-06-30
        data_dict = {
            'train': combined_data.loc[(combined_data[self.config['date_col']] >= pd.to_datetime(train_start)) & (combined_data[self.config['date_col']] <= pd.to_datetime(train_end))], 
            'test': combined_data.loc[(combined_data[self.config['date_col']] >= pd.to_datetime(test_start)) & (combined_data[self.config['date_col']] <= pd.to_datetime(test_end))]
        }
        self.logger.info(f'Successfully split data into train and test sets for long-horizon forecasting')
            
        # Save data to s3
        for data_split in data_splits:
            self.s3_helper.to_parquet(
                data=data_dict[data_split],
                obj_key=os.path.join(self.config['long']['output_key'], f'{data_split}.parquet')
            )
            self.logger.info(f'Successfully written {data_split} data to s3 for long-horizon forecasting')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Process data for forecasting tasks')
    parser.add_argument('--config', type=str, default='config', help='This is the path to the configuration file, which defaults to the config directory in src')
    parser.add_argument('--process_task_type', type=str, choices=['short', 'long'], help='Forecasting horizon determines the type of processing task')
