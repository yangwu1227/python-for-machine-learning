...
class STLTrainer(LongTrainer):
    ...
    def _create_model(self) -> ColumnEnsembleForecaster:
        ...
        for target in self.config['targets']:
            ...
            target_pipeline = TransformedTargetForecaster([
                ...
                ('detrend', OptionalPassthrough(transformer=Detrender(), passthrough=False)),
                ('deseasonalize', OptionalPassthrough(transformer=Deseasonalizer(sp=self.config[self.horizon]['m']), passthrough=False)),
                ('slt', STLForecaster(
                    sp=self.config[self.horizon]['m'],
                    robust=True,
                    forecaster_trend=StatsForecastAutoETS(season_length=self.config[self.horizon]['m']),
                    forecaster_seasonal=StatsForecastAutoETS(season_length=self.config[self.horizon]['m']),
                    forecaster_resid=StatsForecastAutoETS(season_length=self.config[self.horizon]['m'])
                ))
            ])       
    ...
    def refit_and_forecast(self) -> Dict[str, Union[pd.DataFrame, Dict[str, pd.DataFrame]]]:
        """
    ...
    @staticmethod
    def extract_prediction_intervals(self, pi: pd.DataFrame, level: float) -> Dict[str, pd.DataFrame]:
        """
        ...
        """
        raise NotImplementedError('The STL forecaster does not support prediction intervals')
