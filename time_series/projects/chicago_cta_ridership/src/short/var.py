...
class VARTrainer(BaseTrainer):
    ...
    def _create_model(self) -> ForecastingPipeline:
        ...
        target_pipeline = TransformedTargetForecaster([
            ...
            ('differencing', OptionalPassthrough(Differencer(lags=[self.config[self.horizon]['m'], 1]))),
            ('var', VAR(
        ...
        forecasting_pipeline = ForecastingPipeline([
            ('date_features', OptionalPassthrough(DateTimeFeatures(
    ...
    def forecast(self, level: float = 0.95) -> Dict[str, Union[pd.DataFrame, Dict[str, pd.DataFrame]]]:
        ...
        with warnings.catch_warnings():
            ...
            pi = self.__class__.extract_prediction_intervals(
                pi=self.grid_search.predict_interval(fh=self.test_fh, X=self.X_test, coverage=level),
                level=level
        )
    ...
    def refit_and_forecast(self, level: float = 0.95) -> Dict[str, Union[pd.DataFrame, Dict[str, pd.DataFrame]]]:
        ...
        oos_X = pd.Series(
            data=np.zeros(self.config[self.horizon]['forecast_horizon']), # All 0 vector with length equal to the forecast horizon
            index=pd.date_range(
                start=self.y_full.index[-1], # Start from the last date in the entire data set
                periods=self.oos_fh[-1], # Move forward by the 'forecast horizon' of periods
                freq=self.config[self.horizon]['freq'] # Use the same frequency as the data set
                ),
        ...
        pi = self.__class__.extract_prediction_intervals(
            pi=self.best_forecaster.predict_interval(fh=self.oos_fh, X=oos_X, coverage=level),
            level=level
        )
    ...
    def diagnostics(self, 
        ...
        if full_model:
            ...
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                residuals = self.best_forecaster.predict_residuals(y=self.y_full, X=self.X_full)
        else:
            if self._attribute_is_none('grid_search'):
                raise ValueError('The grid search attribute is None, please run `cross_validate` method first')
            with warnings.catch_warnings():
                warnings.simplefilter('ignore')
                residuals = self.grid_search.best_forecaster_.predict_residuals(y=self.y_train, X=self.X_train)
