...
def near(pt, alist, dist=15):
    ...
    '''
    for a in alist:
        x, y = a.ao.get_position()  # Use (bot left, bot right) data coords, not relative
        x = x - 5  
        y = y + 2.5
...
class plt_network:
    ...
    def __init__(self, fn, image, out=None):
        ...
        self.axreveal = plt.axes([0.55, 0.02, 0.15, 0.075])  # Use [left, bottom, width, height]
    ...
    def onclick(self, event):
        ...
        if inbox:
            self.san.append(box)
            graphBox = self.fig.add_axes([0.225, 0.02, 0.2, 0.075])  # Use [left, bottom, width, height]
