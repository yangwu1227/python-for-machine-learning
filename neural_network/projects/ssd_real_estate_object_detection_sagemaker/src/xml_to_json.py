...
def parse_file(
    filename: str, image_id: int, mappings: Dict[str, int]
...
def generate_annotations(
    directories: List[str], output_files: List[str], mappings: Dict[str, int]
) -> None:
    """
...
def create_directories(
    data_dir: str, image_format: str = "jpeg", val_split: bool = False
) -> None:
    """
    ...
    train_images = [
        ...
        if train_image.replace(image_format, "xml") in train_xmls
    ]
    test_images = [
        test_image
        for test_image in test_images
        if test_image.replace(image_format, "xml") in test_xmls
    ]
    ...
    train_xmls = [
        ...
        if train_xml.replace("xml", image_format) in train_images
    ]
    test_xmls = [
        test_xml
        for test_xml in test_xmls
        if test_xml.replace("xml", image_format) in test_images
    ]
    ...
    if val_split:
        ...
        val_xmls = [
            train_xml
            for train_xml in train_xmls
            if train_xml.replace("xml", image_format) in val_images
        ]
    ...
    list_of_image_files = (
        ...
        if val_split
        else [train_images, test_images]
    )
    list_of_xml_files = (
        [train_xmls, val_xmls, test_xmls] if val_split else [train_xmls, test_xmls]
    )

    for sub_dir, image_files, xml_files in zip(
        sub_dirs, list_of_image_files, list_of_xml_files
    ):
...
def plot_bbox(data_dir: str, image_name: str) -> None:
    ...
    # Get image data
    image_data = next(
        (image for image in annotations["images"] if image["file_name"] == image_name),
        None,
    )
...
def main() -> None:
    ...
    initialize(
        version_base="1.2", config_path="config", job_name="xml_to_json_annotations"
    )
    ...
    for file_name in file_names:
        ...
        # Unzip the file
        subprocess.run(
            f"unzip -q {raw_data_dir}/{file_name} -d {raw_data_dir}", shell=True
        )
    ...
    xml_dirs = [
        os.path.join(raw_data_dir, sub_dir, "annotations") for sub_dir in sub_dirs
    ]
    output_dirs = [
        os.path.join(raw_data_dir, sub_dir, "annotations.json") for sub_dir in sub_dirs
    ]
