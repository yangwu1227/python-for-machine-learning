...
def generate_data():
    ...
    def _generate_data(
        batch_size: int, num_months: int, num_predictions: int
        ...
        X_train = tf.random.uniform(
            shape=(batch_size, num_months), minval=-5, maxval=5, dtype=tf.float32
        )
...
class TestCustomGRUCell(object):
    ...
    def test_save_load_model(self, generate_data):
        ...
        x = tf.keras.layers.RNN(
            cell=CustomGRUCell(
                layer_norm=tf.keras.layers.LayerNormalization(), units=2
            ),
        ...
        model.compile(
            ...
            loss=SymmetricMeanAbsolutePercentageError(
                reduction=tf.keras.losses.Reduction.AUTO, name="smape"
            ),
        )
        ...
        assert tf.reduce_all(
            tf.equal(model.predict(X_train), loaded_model.predict(X_train))
        )
