...
def generate_data():
    ...
    def _generate_data(
        batch_size: int, num_predictions: int
    ) -> Dict[
        str, Union[Tuple[tf.Tensor, tf.Tensor], Tuple[torch.Tensor, torch.Tensor]]
    ]:
        """
        ...
        """
        y_true_tf = tf.random.uniform(
            shape=(batch_size, num_predictions), minval=-5, maxval=5, dtype=tf.float32
        )
        y_pred_tf = tf.random.uniform(
            shape=(batch_size, num_predictions), minval=-5, maxval=5, dtype=tf.float32
        )
...
class TestCustomLoss(object):
    def test_init(self):
        ...
        loss_fn = SymmetricMeanAbsolutePercentageError(
            reduction="sum_over_batch_size", name="smape"
        )
    ...
    def test_from_config(
        self, config: Dict[str, str] = {"reduction": "sum", "name": "smap"}
    ):
        """
    ...
    def test_call(self, batch_size, num_predictions, generate_data):
        ...
        loss_fn_tf = SymmetricMeanAbsolutePercentageError(
            reduction="sum_over_batch_size", name="smape"
        )
        ...
        sum_of_instance_losses_torch = (
            (loss_torch * (y_true_torch.shape[0] * y_true_torch.shape[1]))
            .detach()
            .numpy()
            .item()
        )
        ...
        assert isclose(
            sum_of_instance_losses_torch, sum_of_instance_losses_tf, rtol=1e-5
        )
