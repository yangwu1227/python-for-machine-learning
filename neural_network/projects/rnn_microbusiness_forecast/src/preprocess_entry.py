        ...
        train_data = train_data \
            ...
            .with_columns(
                pl.when(pl.col('year') == train_data_year) \
                  .then(pl.col('microbusiness_density') * (pl.col('est_pop') / pl.col('est_pop_2021'))) \
...
def split_counties(train_data: pl.DataFrame, threshold: int = 50) -> Dict[str, List[int]]:
    """
    ...
    train_data = train_data.groupby('cfips') \
    ...
    large_counties_cfips = train_data.filter((pl.col('est_pop_2021') >= quantile_val_10) & (pl.col('max_density_by_cfips') < threshold)) \
                                     .select(pl.col('cfips')) \
                                     .unique()['cfips'].to_list()
                                     
    small_counties_cfips = list(set(train_data['cfips'].to_list()) - set(large_counties_cfips))
                      
    return {'large_counties': large_counties_cfips, 'small_counties': small_counties_cfips}

def convert_to_numpy(train_data: pl.DataFrame, 
                     cfips: Dict[str, List[int]],
                     num_predictions: int = 5,
                     series_len: int = 18) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    large_counties = train_data.filter(pl.col('cfips').is_in(cfips['large_counties'])) \
                               .sort('cfips') \
                               .select(pl.col('microbusiness_density')) \
                               .to_numpy() \
                               .reshape(num_counties, 41)[:, -1 * training_window:] # Keep only the last 'training_window' months
    ...
    X_train = np.zeros((num_counties * series_len, training_window - series_len - (num_predictions - 1)))
    ...
    for j in range(num_counties):
        ...
        for k in range(series_len):
            ...
            X_train[i,] = large_counties[j, k:k + (training_window - series_len - (num_predictions - 1))]
            # The target should be the next 5 months after the window of length `series_len - num_predictions` above
            y_train[i,] = large_counties[j, k + (training_window - series_len - (num_predictions - 1)):k + (training_window - series_len + 1)]
            
    return X_train, y_train, large_counties[:, -(series_len - num_predictions):]

# ----------------------------- Convert to ratios ---------------------------- #

def compute_ratios(X: np.ndarray, 
                   y: np.ndarray, 
                   num_predictions: int = 5,
                   series_len: int = 18) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def main():
    ...
    train_data = density_adjustment(train_data, os.path.join(config['processing_job_input'], 'census-data'))
    ...
    X_train_densities, y_train_densities, large_counties = convert_to_numpy(train_data, large_small_counties, num_predictions=config['gru']['num_predictions'], series_len=config['gru']['series_len'])


    logger.info('Converting training data to ratios...')
    X_train, y_train = compute_ratios(X_train_densities, y_train_densities, num_predictions=config['gru']['num_predictions'], series_len=config['gru']['series_len'])


    logger.info('Saving training data to output directory...')
    train_data = pl.DataFrame(
        data=np.concatenate([X_train, y_train], axis=1),
        schema=['baseline_raw_density'] + [f'x{i}' for i in range(X_train.shape[1] - 1)] + [f'y{i}' for i in range(y_train.shape[1])]
    )
    train_data = train_data.with_columns(
        pl.Series(
            name='cfips',
            values=np.repeat(sorted(large_small_counties['large_counties']), config['gru']['series_len'])
            )
    )

    num_of_months = config['gru']['series_len'] - config['gru']['num_predictions']
    logger.info(f'Saving last {num_of_months} months of raw densities to disk for predictions...')
    ...
    if not args.test_mode:
        ...
        np.save(os.path.join(config['processing_job_output'], 'densities_train_forecast.npy'), large_counties)

        np.save(os.path.join(config['processing_job_output'], 'densities_target.npy'), y_train_densities)
