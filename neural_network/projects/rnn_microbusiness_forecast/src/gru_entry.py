...
class SymmetricMeanAbsolutePercentageError(tf.keras.losses.Loss):
    ...
    def call(self, y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        denominator = tf.clip_by_value(tf.abs(y_true) + tf.abs(y_pred), clip_value_min=tf.keras.backend.epsilon(), clip_value_max=float('inf'))

        # Sum across the `num_predictions` dimension and multiply by 2
        instance_losses = tf.multiply(2.0, tf.math.reduce_sum(numerator / denominator, axis=-1))
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> SymmetricMeanAbsolutePercentageError:
        """
...
class CustomGRUCell(tf.keras.layers.GRUCell):
    ...
    def get_config(self) -> Dict[str, Any]:
        ...
        config = {
            'layer_norm': tf.keras.layers.serialize(self.layer_norm)
        }
...
class GRUTrainer(object):
    ...
    def _create_model(self) -> tf.keras.Model:
        ...
        default_gru_cell = partial(
            ...
            kernel_regularizer=tf.keras.regularizers.l2(l2=self.hyperparameters['gru_kernel_l2_factor']),
            # Applied to the weight matrices that transform hidden state h_t-1
            recurrent_regularizer=tf.keras.regularizers.l2(l2=self.hyperparameters['gru_recurrent_l2_factor']),
        ...
        inputs = tf.keras.Input(shape=(self.config['gru']['series_len'] - self.config['gru']['num_predictions'] - 1, 1), name='input_layer')
        # Normalize the inputs
        normalized_inputs = tf.keras.layers.LayerNormalization(name='layer_norm_input')(inputs)

        # Stacked GRU layers
        x = tf.keras.layers.RNN(
            cell=default_gru_cell(layer_norm=tf.keras.layers.LayerNormalization(name='layer_norm_gru_0'), units=self.hyperparameters['gru_units_0'], name='gru_cell_0'),
            return_sequences=False if self.hyperparameters['gru_num_layers'] == 1 else True, # If using a single GRU layer, them this layer should only return the last output `h_t`
            name='gru_layer_0'
        )(normalized_inputs)
        ...
        for i in range(1, self.hyperparameters['gru_num_layers']):
            x = tf.keras.layers.RNN(
                cell=default_gru_cell(layer_norm=tf.keras.layers.LayerNormalization(name=f'layer_norm_gru_{i}'), units=self.hyperparameters[f'gru_units_{i}'], name=f'gru_cell_{i}'),
                # If this is the last GRU layer, set return_sequences = False
                return_sequences=False if i == (self.hyperparameters['gru_num_layers'] - 1) else True,
                name=f'gru_layer_{i}'
            )(x)
        
        # Dense layer
        outputs = tf.keras.layers.Dense(units=self.config['gru']['num_predictions'], name='output_layer')(x)
    ...
    def _generate_sample_weights(self, 
    ...
    def _fit(self, 
             ...
             fold_sample_weights: np.ndarray) -> tf.keras.Model:
        """
    ...
    def cross_validate(self) -> None:
        ...
        oof_preds = np.zeros(shape=(self.train_data.shape[0], self.config['gru']['num_predictions']))
        # Training and target data
        X_train = self.train_data.select([col for col in self.train_data.columns if col.startswith('x')]).to_numpy()
        y_train = self.train_data.select([col for col in self.train_data.columns if col.startswith('y')]).to_numpy()
        ...
        for fold, (train_indices, val_indices) in enumerate(gkf_splitter):
            ...
            fold_model.save(os.path.join(self.model_dir, f'fold_{fold + 1}_model.keras'))
        ...
        oof_predicted_densities[:, 0] = baseline_raw_density.to_numpy() * oof_preds[:, 0]
        for k in range(4):
            oof_predicted_densities[:, k + 1] = oof_predicted_densities[:, k] * oof_predicted_densities[:, k + 1]

        # Compute the SMAPE loss between predicted densities and target densities
        loss_fn = self._create_loss_fn()
        avg_smape_loss = loss_fn(y_true=self.target_raw_densities, y_pred=oof_predicted_densities).numpy()
...
if __name__ == '__main__':
    ...
    if args.test_mode:
        ...
        train_data = pl.read_csv(os.path.join(args.train, 'train.csv'))[:2 * config['gru']['series_len']]

        # Target raw densities
        target_raw_densities = np.load(os.path.join(args.train, 'densities_target.npy'))[:2 * config['gru']['series_len']]
    else:
