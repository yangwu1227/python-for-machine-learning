...
from src.custom_utils import get_logger, AugmentationModel, TuningVisualizer, ErrorAnalyzer
...
def augmentation_layers():
    ...
    return {
        ...
        'invalid_non_dict_input': ('RandomFlip', 'RandomRotation', 'RandomZoom', 'RandomContrast'),
...
class TestAugmentationLayer:
    ...
    def test_constructor(self, augmentation_layers):
        ...
        augmentation_model = AugmentationModel(aug_params=augmentation_layers['valid_dict_input'])
    ...
    def test_exceptions(self, augmentation_layers):
        ...
        with pytest.raises(TypeError):
            AugmentationModel(aug_params=augmentation_layers['invalid_non_dict_input']).build_augmented_model()

        # Keras raises an exception if the layer name is invalid
        with pytest.raises(ValueError):
            AugmentationModel(aug_params=augmentation_layers['invalid_layer_name']).build_augmented_model()

        # Keras raises an exception if the layer config is invalid
        with pytest.raises(TypeError):
            AugmentationModel(aug_params=augmentation_layers['invalid_layer_config']).build_augmented_model()
...
def visualizer():
    ...
    num_params = ['param2']
    cat_params = ['param1']
    return TuningVisualizer(tune_data=tune_data, num_params=num_params, cat_params=cat_params)
...
class TestTuningVisualizer:
    ...
    [
        ...
        (pd.DataFrame({'param1': ['low', 'high', 'low', 'high']}), ('param1', 'param2'), ['param2']),
        # Case 3: cat_params is not a list
        (pd.DataFrame({'param1': ['low', 'high', 'low', 'high']}), ['param1'], ('param1', 'param2')),
        # Case 4: num_params accidently inputed as indices
        (pd.DataFrame({'param1': ['low', 'high', 'low', 'high']}), [1, 2, 3], ['param1']),
        # Case 5: cat_params accidently inputed as indices
        (pd.DataFrame({'param1': ['low', 'high', 'low, high', 'high']}), ['param1'], [1, 2, 3])
    ],
    scope='function'
    )
    ...
    def test_properties_type(self, tune_data, num_params, cat_params):
        ...
        with pytest.raises(TypeError):
            TuningVisualizer(tune_data=tune_data, num_params=num_params, cat_params=cat_params)
    ...
    def test_cat_mapping(self, visualizer):
        ...
        assert visualizer._cat_mapping(cat_params=visualizer.cat_params) == [{'low': 0, 'high': 1}]
    ...
    def test_plot_parallel_coordinate(self, visualizer):
        ...
        assert isinstance(visualizer.plot_parallel_coordinate(columns=visualizer.num_params + visualizer.cat_params), go.Figure)

        # Test that the method returns an Image object in non-interactive mode
        assert isinstance(visualizer.plot_parallel_coordinate(columns=visualizer.num_params + visualizer.cat_params, static=True), Image)
...
def label_map():
    return {
        'backyard': 0, 'bathroom': 1, 'bedroom': 2, 'diningRoom': 3, 
        'frontyard': 4, 'hall': 5, 'kitchen': 6, 'livingRoom': 7, 'plan': 8
    }
...
class TestErrorAnalyzer:
    ...
    @pytest.mark.parametrize(
        'class_label, path',
        [
            ('bathroom', 'test_plots/bathroom.png')
        ],
        scope='function'
    )
    ...
    def test_plot(self, test_data, label_map, class_label, path):
        ...
        """
        # Test that the method returns a plotly figure in interactive mode
        error_analyzer = ErrorAnalyzer(test_data['y_val'], test_data['X_val'], test_data['y_pred'], label_map)
