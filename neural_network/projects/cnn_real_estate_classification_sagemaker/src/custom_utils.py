...
def classification_report(y_true: np.ndarray, y_pred: np.ndarray, labels: List[str]) -> Tuple[pd.DataFrame, Dict[str, float]]:
    """
    ...
    agg_metrics_keys = ['avg_pre', 'avg_rec', 'avg_spe', 'avg_f1', 'avg_geo', 'avg_iba', 'total_support']
    ...
    clf_report_frame = pd.DataFrame(clf_report_dict).transpose().loc[class_indices].round(4)

    # Add the class names
    clf_report_frame.index = labels

    # Change column names to be more readable
    clf_report_frame.columns = ['precision', 'recall', 'specificity', 'f1', 'geometric_mean', 'index_balanced_accuracy', 'support']
...
class TuningVisualizer(object):
    ...
    """
    def __init__(self, tune_data: pd.DataFrame, num_params: List[str], cat_params: List[str]):
        """
    ...
    def num_params(self, num_params):
        if not isinstance(num_params, list) or not all(isinstance(param, str) for param in num_params):
            raise TypeError('The numerical parameters must be supplied as a list of strings')
    ...
    def cat_params(self, cat_params):
        if not isinstance(cat_params, list) or not all(isinstance(param, str) for param in cat_params):
            raise TypeError('The categorical parameters must be supplied as a list of strings')
    ...
    def _cat_mapping(self, cat_params: List[str]) -> List[Dict[str, int]]:
        ...
        for param in cat_params:
            ...
            cat_mappings.append({categorical_key: integer_encoding for integer_encoding, categorical_key in enumerate(self.tune_data[param].unique())})
    ...
    def plot_parallel_coordinate(self, columns: List[str], target: str = 'FinalObjectiveValue', static: bool = False, figsize: Tuple[float, float] = (1000, 800), **kwargs) -> go.Figure:
        """
        ...
        **kwargs: Dict[str, Any]
        ...
        if len(cat_params) > 0:
            ...
            for i, col in enumerate(cat_params):
                ...
                cat_dicts.append(dict(label=col, values=tune_data[col], tickvals=list(cat_mappings[i].values()), ticktext=list(cat_mappings[i].keys()))) 
        ...
        fig = go.Figure(go.Parcoords(
            line = dict(color = tune_data[target], colorscale = kwargs['colorscale'] if 'colorscale' in kwargs.keys() else 'viridis', showscale = True),
...
class ErrorAnalyzer(object):
    ...
    """
    def __init__(self, y_true: np.ndarray, images: np.ndarray, y_pred: np.ndarray, label_mapping: Dict[str, int]):
        """
    ...
    def _clf_by_class(self) -> Dict[str, np.ndarray]:
        ...
        for encode, label in zip(self.class_encode, self.class_label):
            ...
            correct_indices = np.where((self.y_true == encode) & (self.y_true == np.argmax(self.y_pred, axis=1)))[0]
            # Indices of misclassified samples
            mis_clf_indices = np.where((self.y_true == encode) & (self.y_true != np.argmax(self.y_pred, axis=1)))[0]
            clf_by_class[label] = {'correctly_classified': correct_indices, 'misclassified': mis_clf_indices}

        return clf_by_class

    def plot_mis_clf(self, class_label: str, sample_mis_clf: int = 10, sample_correct_clf: int = 5, figsize: Tuple[int, int] = (12, 10), **kwargs) -> None:
        """
        ...
        **kwargs: Dict[str, Any]
        ...
        if sample_mis_clf > 10 or sample_correct_clf > 5:
            raise ValueError('The arguments `sample_mis_clf` must be less than or equal to 10 and `sample_correct_clf` must be less than or equal to 5.')
        ...
        mis_clf_indices = np.random.choice(mis_clf_indices, size=sample_mis_clf, replace=False)
        correct_indices = np.random.choice(correct_clf_indices, size=sample_correct_clf, replace=False)
        ...
        for i, idx in enumerate(mis_clf_indices):
            ...
            pred_label = self.class_label[self.class_encode.index(np.argmax(self.y_pred[idx]))]
