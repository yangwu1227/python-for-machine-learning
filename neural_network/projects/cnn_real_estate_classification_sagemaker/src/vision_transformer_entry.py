...
class ShiftedPatchTokenizer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.projection = tf.keras.layers.Dense(
            units=self.projection_dim, name="projection_layer"
        )
    ...
    def shift_crop_pad(self, images: tf.Tensor, mode: str) -> tf.Tensor:
        ...
        if mode == "left-up":
            ...
            raise ValueError(
                f"Invalid mode: {mode}, must be one of the following: left-up, left-down, right-up, right-down"
            )
...
class MultiHeadAttentionLSA(tf.keras.layers.MultiHeadAttention):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.temperature = tf.Variable(
            initial_value=sqrt(self._key_dim), trainable=True
        )
...
def mlp(
    x: tf.Tensor, hidden_units: List[int], dropout_rate: float, mode: str
) -> tf.Tensor:
    """
...
class VisionTransformerTrainer(BaseTrainer):
    ...
    def _create_model(self) -> tf.keras.Model:
        ...
        num_patches = (
            self.config["image_size"] // self.hyperparameters["spt_patch_size"]
        ) ** 2
        ...
        encoded_visual_tokens = PatchEncoder(
            num_patches=num_patches,
            projection_dim=self.hyperparameters["spt_projection_dim"],
            name="patch_encoder",
        )(visual_tokens)
        ...
        diag_attn_mask = tf.expand_dims(
            (1 - tf.eye(num_patches, dtype=tf.int8)), axis=0
        )
        # Transformer encoder block
        for block in range(self.hyperparameters["transformer_num_layers"]):
            # Layer normalization 1
            norm_1 = tf.keras.layers.LayerNormalization(
                epsilon=1e-6, name=f"transformer_block_{block}_layer_normalization_1"
            )(encoded_visual_tokens)
            ...
            )([attention_output, encoded_visual_tokens])
            # Layer normalization 2
            norm_2 = tf.keras.layers.LayerNormalization(
                epsilon=1e-6, name=f"transformer_block_{block}_layer_normalization_2"
            )(skip_con_1)
            # MLP
            norm_2 = mlp(
                x=norm_2,
                hidden_units=[
                    self.hyperparameters["transformer_mlp_multiple_0"]
            ...
            # Skip connection 2
            encoded_visual_tokens = tf.keras.layers.Add(
        ...
        representation = tf.keras.layers.LayerNormalization(
            epsilon=1e-6, name="layer_normalization"
        )(encoded_visual_tokens)
        representation = tf.keras.layers.Flatten(name="flatten")(representation)
        representation = tf.keras.layers.Dropout(
            rate=self.hyperparameters["dense_dropout_rate"], name="dropout"
        )(representation)

        # MLP head
        hidden_units = [
            self.hyperparameters[f"dense_units_{i}"]
    ...
    def fit(self) -> None:
        ...
        self.logger.info(
            f"Number of trainable parameters for training: {trainable_params_count}"
        )
