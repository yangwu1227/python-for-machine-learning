...
def generate_monkey_saddle_data(start: float = -np.e, 
                                end: float = np.e, 
                                num_samples: int = 500, 
                                **kwargs) -> np.ndarray:
    """
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description='PyTorch LSTM and GRU for Monkey Saddle')
    parser.add_argument('--samples', type=int, default=100, help='Number of samples to generate on the real line')
    ...
    parser.add_argument('--hidden_size', type=int, default=20, help='The number of features in the hidden state h')
    parser.add_argument('--num_layers', type=int, default=3, help='The number of recurrent layers')
    ...
    data = generate_monkey_saddle_data(start=args.start, end=args.end, num_samples=args.samples, theta=args.theta, flip=args.flip, translate=args.translate, scale=args.scale)
