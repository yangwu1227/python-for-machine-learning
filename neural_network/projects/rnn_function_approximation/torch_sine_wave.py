...
def generate_2d_sine_wave(
    samples: int, freq_x: float = 1, freq_y: float = 1, noise: float = 0.1
) -> np.ndarray:
    """
    ...
    sinewave_2d = (
        np.sin(freq_x * x)[:, None]
        + np.sin(freq_y * y)[None, :]
        + noise * np.random.randn(samples, samples)
    )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="PyTorch LSTM and GRU for 2D Sine Wave"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="The frequency (number of times a wave repeats a cycle) of the sine wave in the x direction",
    )
    parser.add_argument(
        "--freq_y",
        type=float,
        default=1,
        help="The frequency (number of times a wave repeats a cycle) of the sine wave in the y direction",
    )
    parser.add_argument(
        "--noise",
        type=float,
        default=0.1,
        help="The amount of noise (scalar factor) to add to the sine wave",
    )
    parser.add_argument(
        "--input_size",
        type=int,
    ...
    parser.add_argument(
        "--num_layers", type=int, default=2, help="The number of recurrent layers"
    )
    parser.add_argument(
