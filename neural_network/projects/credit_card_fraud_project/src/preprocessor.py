...
class LogTransformer(keras.layers.Layer):
    ...
    def call(self, inputs: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        return inputs.assign(amount=np.log(inputs['amount'] + 1e-6))  # Add small constant to avoid log(0)
...
class CustomScaler(keras.layers.Layer):
    ...
    def build(self, input_shape: tf.TensorShape):
        ...
        """
        self.mean = self.add_weight(name='mean', shape=(
            input_shape[-1],), initializer='zeros', trainable=False)
        self.std = self.add_weight(name='std', shape=(
            input_shape[-1],), initializer='zeros', trainable=False)

    def call(self, inputs: Union[pd.DataFrame, np.array, tf.Tensor], training=None) -> tf.Tensor:
        """
