...
class FineTuneTrainer(BaseTrainer):
    ...
    def _create_model(self) -> Dict[str, tf.keras.Model]:
        ...
        inputs = tf.keras.Input(shape=(self.config['image_size'], self.config['image_size'], 3), name='input_layer')
    ...
    def _recompile_model(self, model: tf.keras.Model, conv_base: tf.keras.Model) -> tf.keras.Model:
        ...
        optimizer = self._create_optimizer(learning_rate=self.hyperparameters['opt_learning_rate'] / 10)
        loss_fn = self._create_loss_fn()
        metrics = self._create_metrics()
        model.compile(
            optimizer=optimizer,
    ...
    def fit(self) -> None:
        ...
        self.logger.info(f'Best validation loss after fine-tuning: {early_stopping.best}')
...
if __name__ == '__main__':
    ...
    initialize(version_base='1.2', config_path='config', job_name='fine_tune_resnet50v2')
