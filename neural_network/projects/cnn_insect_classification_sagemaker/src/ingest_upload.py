...
def downsample_class(args: Tuple[str, int, int]) -> None:
    ...
    image_files = [os.path.join(class_dir, image_file) for image_file in os.listdir(class_dir)]
...
def downsample(directory: str, max_samples: int, random_seed: int) -> None:
    ...
    class_dirs = dict(sorted({int(int_label): os.path.join(directory, int_label) for int_label in os.listdir(directory)}.items()))

    # Classes to delete samples from
    class_to_delete = {int_label: path for int_label, path in class_dirs.items() if len(os.listdir(path)) > max_samples}

    if len(class_to_delete) != 0:
        with Pool() as p:
            p.map(downsample_class, [(class_dir, max_samples, random_seed) for int_label, class_dir in class_to_delete.items()])
...
def count_samples(directory: str) -> Dict[str, int]:
    ...
    sample_counts = {cls: len(os.listdir(os.path.join(directory, cls))) for cls in classes}
...
def calculate_class_weights(class_counts: Dict[str, int]) -> Dict[int, float]:
    ...
    class_weights = {int(class_label): total_samples / count for class_label, count in class_counts.items()}
...
def main() -> int:
    ...
    directories = {directory: os.path.join(raw_data_dir, directory) for directory in ['train', 'val', 'test']}
    ...
    for dir_key in directories:
        ...
        percentages = {cls: (count / total_counts[dir_key]) * 100 for cls, count in class_counts.items()}
    ...
    train_bar = ax.bar(indices, class_percentages['train'].values(), width=bar_width, label=f'Train (Total: {total_counts["train"]})')
    val_bar = ax.bar([i + bar_width for i in indices], class_percentages['val'].values(), width=bar_width, label=f'Validation (Total: {total_counts["val"]})')
    test_bar = ax.bar([i + 2 * bar_width for i in indices], class_percentages['test'].values(), width=bar_width, label=f'Test (Total: {total_counts["test"]})')
