...
class TabTransformerTrainer(object):
    ...
    def _create_loss_fn(self) -> tf.keras.losses.Loss:
        ...
        if self.hyperparameters['use_focal_loss']:
            loss_fn = tf.keras.losses.BinaryFocalCrossentropy(
                apply_class_balancing=self.hyperparameters['loss_apply_class_balancing'],
    ...
    def _create_inputs(self) -> tf.keras.Input:
        ...
        """
        inputs = {}
        feature_names = self.config['tf_keras']['num_feat'] + list(self.config['tf_keras']['cat_feat_vocab'].keys())
    ...
    def _encode_inputs(self, inputs: tf.keras.Input) -> Tuple[List[tf.Tensor], List[tf.Tensor]]:
        """
        ...
        for feature_name in inputs:
            ...
            if feature_name in self.config['tf_keras']['cat_feat_vocab'].keys():
                ...
                embedding = tf.keras.layers.Embedding(
                    input_dim=len(vocabulary), output_dim=self.hyperparameters['transformer_embedding_dims']
                )
    ...
    def _create_model(self, epsilon: float = 1e-16) -> tf.keras.Model:
        ...
        column_embedding = tf.keras.layers.Embedding(
            input_dim=num_cat_feat, output_dim=self.hyperparameters['transformer_embedding_dims']
        )
        ...
        for block in range(self.hyperparameters['transformer_num_layers']):
            ...
            )(query=encoded_categorical_features, value=encoded_categorical_features, key=encoded_categorical_features)

            # Skip connection 1
            skip_con_1 = tf.keras.layers.Add(name=f'transformer_block_{block}_skip_connection_1')(
                [attention_output, encoded_categorical_features]
            )
            # Layer normalization 1
            norm_1 = tf.keras.layers.LayerNormalization(epsilon=epsilon, name=f'transformer_block_{block}_layer_normalization_1')(skip_con_1)
            ...
            # Skip connection 2
            skip_con_2 = tf.keras.layers.Add(name=f'transformer_block_{block}_skip_connection_2')([mlp_output, norm_1])
        ...
        numerical_features = tf.keras.layers.LayerNormalization(epsilon=epsilon, name=f'numerical_layer_normalization')(numerical_features)
        # Concatenate the input for the final MLP block (batch_size, (num_cat_feat * embedding_dims) + num_cont_feat)
        features = tf.keras.layers.concatenate(inputs=[categorical_features, numerical_features], axis=-1)
        ...
        mlp_hidden_units = [factor * features.shape[-1] for factor in self.hyperparameters['mlp_hidden_units_multiples']]
    ...
    def fit(self) -> Tuple[tf.keras.Model, float]:
        ...
        if not retrain_on_full:
            tensorboard_local_dir = f'/tmp/tensorboard/{self.job_name}_{self.trial_number}'
            tensorboard = tf.keras.callbacks.TensorBoard(
                log_dir=tensorboard_local_dir
            )
        ...
        model.fit(
            ...
            validation_steps=self.hyperparameters['fit_validation_steps'] if not retrain_on_full else None
        )

        if not retrain_on_full:
            subprocess.run(
                ['aws', 's3', 'cp', tensorboard_local_dir, f's3://{self.config["s3_bucket"]}/{self.config["s3_key"]}/tensorboard_logs/{self.job_name}_{self.trial_number}', '--recursive']
            )
...
def tf_objective(trial: optuna.Trial,
    ...
    hyperparameters = {
        ...
        'transformer_embedding_dims': trial.suggest_categorical('transformer_embedding_dims', [2**power for power in range(5, 8)]),
        'transformer_dropout_rate': trial.suggest_float('transformer_dropout_rate', 1e-2, 0.5),
        ...
        'adam_learning_rate': trial.suggest_float('adam_learning_rate', 1e-4, 1e-1, log=True),
    ...
    for i in range(hyperparameters['mlp_num_hidden_layers']):
        ...
        hyperparameters['mlp_hidden_units_multiples'].append(trial.suggest_int(f'mlp_hidden_units_multiple_{i}', 2, 4))

    # Conditionally add focal loss hyperparameters
    if hyperparameters['use_focal_loss']:
        hyperparameters['loss_apply_class_balancing'] = trial.suggest_categorical('loss_apply_class_balancing', [True, False])
...
if __name__ == '__main__':
    ...
    from custom_utils import (get_logger, parser, add_additional_args, get_db_url, 
                              create_study, study_report, dataset_from_csv, test_sample)
