...
def fine_tune_unet(logger: logging.Logger,
                   back_bone: str,
    ...
    x = tf.keras.layers.Conv2D(filters=3, kernel_size=(1, 1), kernel_initializer='he_normal')(x) # Map 1 channel to 3 channels
    outputs = conv_base(x, training=False)
    
    model = tf.keras.Model(inputs=inputs, outputs=outputs)
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=opt_params['learning_rate'], clipnorm=opt_params['clipnorm']),
        loss=DiceLoss(),
        metrics=[tf.keras.metrics.MeanIoU(num_classes=2, name='mean_iou'), tf.keras.metrics.Precision(name='precision'), tf.keras.metrics.Recall(name='recall')]
    )
    
    trainable_params_count = np.sum([tf.keras.backend.count_params(w) for w in model.trainable_weights])
    logger.info(f'Number of trainable parameters for decoder training: {trainable_params_count}')
    del trainable_params_count
    
    early_stopper_decoder = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
    ...
    logger.info(f'Best validation dice loss after training decoder blocks with frozen encoder blocks: {early_stopper_decoder.best}')
    ...
    model.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=reduced_learning_rate, clipnorm=opt_params['clipnorm']),
        loss=DiceLoss(),
        metrics=[tf.keras.metrics.MeanIoU(num_classes=2, name='mean_iou'), tf.keras.metrics.Precision(name='precision'), tf.keras.metrics.Recall(name='recall')]
    )
    ...
    early_stopper_fine_tune = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
    ...
    logger.info(f'Best validation dice loss after fine-tuning: {early_stopper_fine_tune.best}')
...
if __name__ == '__main__':
    ...
    train_images, train_masks, val_images, val_masks = load_data({'train': args.train, 'val': args.val}, test_mode=False)
