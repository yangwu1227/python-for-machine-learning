...
def load_data(paths: Dict[str, str], test_mode: bool = False) -> Tuple[np.ndarray]:
    ...
    """
    train_images = np.load(file=os.path.join(
        paths['train'], 'train_images.npy'))
    ...
    if test_mode:
        test_images = np.load(file=os.path.join(
            paths['test'], 'test_images.npy'))
        test_masks = np.load(file=os.path.join(
            paths['test'], 'test_masks.npy'))
...
def unet_model(image_size: Tuple[int, int],
               ...
               num_channels: int = 1) -> tf.keras.Model:
    """
    ...
    # Entry block
    x = default_conv2d(filters=entry_block_filters,
                       kernel_size=entry_block_kernel_size, strides=entry_block_strides)(x)
    ...
    for filters, stride, kernel_size, momentum, pool_size in zip([64, 128, 256],
                                                                 down_sample_strides,
    ...
    for filters, stride, kernel_size, momentum in zip([256, 128, 64, 32],
                                                      up_sample_strides,
                                                      up_sample_kernel_sizes,
                                                      up_sample_batch_norm_momentums):
        x = layers.Activation('relu')(x)
        x = default_conv2d_transpose(
            filters=filters, kernel_size=kernel_size)(x)
        x = layers.BatchNormalization(momentum=momentum)(x)
        ...
        residual = layers.UpSampling2D(
            size=up_sample_size)(previous_block_activation)
    ...
    outputs = default_conv2d(
        filters=num_channels, kernel_size=output_kernel_size, activation='sigmoid')(x)
