...
if __name__ == '__main__':
    ...
    image_paths = sorted([os.path.join(image_dir, image_path) for image_path in os.listdir(image_dir)])
    mask_dir = os.path.join(raw_data_dir, 'masks')
    mask_paths = sorted([os.path.join(mask_dir, mask_path) for mask_path in os.listdir(mask_dir)])
    ...
    dir_dict = {}
    dir_name_sets = ['train', 'train_annotation', 'validation', 'validation_annotation', 'test', 'test_annotation']
    ...
    label_map = {'scale': 1}
    label_file_names = [os.path.join(raw_data_dir, path) for path in ['train_label_map.json', 'validation_label_map.json']]
