...
def report_keras_hpo(tuner: kt.Tuner, X: np.ndarray, y: np.ndarray, logger: logging.Logger) -> Tuple[Dict[str, Any], float] :
    """
...
def plot_1d_curve(y_true:np.ndarray, y_pred: np.ndarray, X: np.ndarray, name: str) -> None:
    """
...
def plot_2d_surfaces(y_true: np.ndarray, y_pred: np.ndarray, X: np.ndarray, name: str, **kwargs) -> Image:
    """
    ...
    **kwargs 
    ...
    fig.add_trace(go.Surface(x=x1, y=x2, z=y_true, name='Original', showscale=False), row=1, col=1)
    fig.add_trace(go.Surface(x=x1, y=x2, z=y_pred, name='Predictions', showscale=False), row=1, col=2)
