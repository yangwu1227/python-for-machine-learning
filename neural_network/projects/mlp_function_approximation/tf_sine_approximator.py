...
def generate_sine_wave_data(start: float = -np.pi, end: float = np.pi, num_samples: int = 2000) -> Tuple[np.ndarray, np.ndarray]:
    """
...
class SineHyperModel(kt.HyperModel):
    ...
    def build(self, hp: kt.HyperParameters) -> tf.keras.Model:
        ...
        kernel_initializer = hp.Choice('kernel_initializer', values=['glorot_normal', 'he_normal', 'lecun_normal'])
        learning_rate = hp.Float('learning_rate', min_value=1e-4, max_value=1e-2, sampling='log')
        optimizer = getattr(tf.keras.optimizers, hp.Choice('optimizer', values=['Adam', 'SGD', 'RMSprop']))(learning_rate=learning_rate)
        ...
        for i in range(hp.Int('n_layers', 1, 3)):
            if i == 0:
                x = tf.keras.layers.Dense(
                    units=hp.Int(f'layer_{i}_n_units', min_value=64, max_value=256, step=16),
                    activation=activation,
                    name=f'layer_{i}',
                )(inputs)
            else:
                units = hp.Int(f'layer_{i}_n_units', min_value=64, max_value=256, step=16)
                x = tf.keras.layers.Dense(units, activation=activation, name=f'layer_{i}')(x)
        outputs = tf.keras.layers.Dense(units=1, activation='linear', name='output_layer')(x)
    ...
    def fit(self, hp: kt.HyperParameters, model: tf.keras.Model, *args, **kwargs) -> float:
        """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_sample', type=int, default=2000, help='Number of training samples to generate')
    parser.add_argument('--start', type=float, default=-np.pi, help='Start of the interval')
    parser.add_argument('--end', type=float, default=np.pi, help='End of the interval')
    parser.add_argument('--max_trials', type=int, default=20, help='Number of hyperparameter trials to run')
    parser.add_argument('--project_name', type=str, default='sine_approximator', help='Name of the project')
    parser.add_argument('--overwrite', type=bool, default=True, help='Whether to overwrite the existing project')
    parser.add_argument('--hypermodel_name', type=str, default='sine_hypermodel', help='Name of the hypermodel')
    parser.add_argument('--verbose', type=int, default=0, help='Verbosity level')
    parser.add_argument('--tensorboard', type=int, default=0, help='Whether to use TensorBoard')
    parser.add_argument('--plot_name', type=str, default='Sine Wave', help='Name of function we are approximating')
    ...
    X, y = generate_sine_wave_data(start=args.start, end=args.end, num_samples=args.num_sample)
    ...
    if args.tensorboard:
        tensorboard = tf.keras.callbacks.TensorBoard(log_dir=os.path.join(output_dir, 'tb-logs-v' + datetime.now().strftime('%y-%m-%d-%H-%M-%S')) )
