...
def monkey_saddle(x: np.ndarray, 
                  y: np.ndarray, 
                  theta: float = np.pi/4, 
                  flip: list[bool] = [False, False, False], 
                  translate: list[float] = [0.0, 0.0, 0.0], 
                  scale: list[float] = [1.0, 1.0, 1.0]) -> np.ndarray:
    """
...
def generate_monkey_saddle_data(start: float = -np.e, end: float = np.e, num_samples: int = 500, **kwargs) -> TensorDataset:
    """
    ...
    **kwargs
    ...
    x, y = np.meshgrid(np.linspace(start, end, num_samples), np.linspace(start, end, num_samples))
...
class MLPRegressor(torch.nn.Module):
    ...
    def __init__(self, num_layers: int, units_per_layer: List[int], initializer: str, input_shape: int = 2):
        """
        Instantiate 
        ...
        # Input layer 
        self.layers.append(torch.nn.Linear(in_features=input_shape, out_features=units_per_layer[0]))
        # Hidden layers start at index 1 and index backwards to get the previous layer's number of units
        for i in range(1, num_layers):
            self.layers.append(torch.nn.Linear(in_features=units_per_layer[i-1], out_features=units_per_layer[i]))
        # Output layer
        self.layers.append(torch.nn.Linear(in_features=units_per_layer[-1], out_features=1))
...
class EarlyStopping:
    ...
    """
    def __init__(self, patience: int = 3, min_delta: float = 0.0, restore_best_model: bool = True):
        """
    ...
    def __call__(self, model: MLPRegressor, val_loss: float, epoch: int) -> Tuple[bool, float]:
        """
        ...
        if (self.best_loss == np.inf) or np.less(self.min_delta, (self.best_loss - val_loss)):
...
def trainer(train: TensorDataset,
          val: TensorDataset,
          num_layers: int, 
          units_per_layer: List[int], 
          activation: str, 
          ...
          gamma: float = 0.8,
          num_workers: int = 2,
          input_shape: int = 2) -> Tuple[MLPRegressor, float]:
    """
    ...
    logger.info(f'Loading data with batch size {batch_size} and {num_workers} workers...')
    ...
    logger.info(f'Using the {loss_fn} loss function and the {optimizer} optimizer with initial learning rate {learning_rate}...')
    ...
    lr_schedulers = torch.optim.lr_scheduler.ExponentialLR(optimizer=optimizer, gamma=gamma)
 
    # ----------------------------- Early stopping ----------------------------- #
    
    early_stopper = EarlyStopping(patience=patience, min_delta=min_delta, restore_best_model=restore_best_model) 
    ...
    for epoch in range(epochs):
        ...
        # Train mode
        model.train()
        for train_batch_index, (X_train_batch, y_train_batch) in enumerate(train_loader):
            ...
            train_running_loss += train_loss.item() # Use loss.item() to return the scalar loss as a float
            train_epoch_steps += 1
            if train_batch_index % 300 == 299 and verbose > 0:
                logger.info(f'Epoch {epoch + 1} | Batch {train_batch_index + 1} | Train Loss {train_running_loss / train_epoch_steps}')
                # Reset running loss
                train_running_loss = 0.0
        ...
        for val_batch_index, (X_val_batch, y_val_batch) in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                if val_batch_index % 300 == 299 and verbose > 0:
                    logger.info(f'Epoch {epoch + 1} | Batch {val_batch_index + 1} | Val Loss {val_running_loss / val_epochs_steps}')
                    val_running_loss = 0.0
        ...
        if epoch > 0:
            ...
            if should_stop:
                ...
                logger.info(f'Best validation loss achieved at epoch {early_stopper.best_epoch + 1}: {best_val_loss}')
...
def objective(trial: optuna.Trial,
    ...
    hyperparameters = {
        ...
        'initializer': trial.suggest_categorical('initializer', ['xavier_normal', 'kaiming_normal']),
        'epochs': trial.suggest_int('epochs', 1, 10),
        'batch_size': trial.suggest_categorical('batch_size', [16, 32, 64]),
        'optimizer': trial.suggest_categorical('optimizer', ['Adam', 'SGD', 'RMSprop']),
        'learning_rate': trial.suggest_float('learning_rate', 1e-4, 1e-2, log=True)
    }
    hyperparameters['units_per_layer'] = [trial.suggest_int(f'num_units_layer_{i}', 64, 256, step=16) for i in range(hyperparameters['num_layers'])]
...
if __name__ == '__main__':
    ...
    # Data generation
    parser.add_argument('--num_samples', type=int, default=500, help='Number of training samples to generate')
    parser.add_argument('--start', type=float, default=-np.e, help='Start of the interval for data generation on the real line')
    parser.add_argument('--end', type=float, default=np.e, help='End of the interval for data generation on the real line')
    parser.add_argument('--theta', type=float, default=np.pi/4, help='Rotation angle in radians')
    parser.add_argument('--flip', type=int, nargs='+', default=[1, 1, 1], help='Whether to flip the saddle function along the corresponding axis--- x, y, z')
    parser.add_argument('--translate', type=float, nargs='+', default=[0.0, 0.0, 0.0], help='Positve or negative floats to translate the saddle function along the corresponding axis--- x, y, z')
    parser.add_argument('--scale', type=float, nargs='+', default=[1.0, 1.0, 1.0], help='Values to scale the saddle function along the corresponding axis--- x, y, z')
    ...
    parser.add_argument('--restore_best_model', type=int, default=1, help='Whether to restore the weights of the model from the epoch with the best loss')
    parser.add_argument('--loss_fn', type=str, default='MSELoss', help='The loss function to use')
    parser.add_argument('--gamma', type=float, default=0.8, help='The learning rate decay factor')
    parser.add_argument('--num_workers', type=int, default=2, help='The number of workers to use for loading the data')
    parser.add_argument('--input_shape', type=int, default=2, help='The shape of the input tensor')
    # Optuna 
    parser.add_argument('--n_trials', type=int, default=5, help='Number of hyperparameter trials to run')
    parser.add_argument('--study_name', type=str, default='saddle_approximator', help='Name of the study')
    ...
    storage = 'sqlite:///{}'.format(os.path.join(output_dir, 'saddle_approximator_hpo.db'))
    study = create_study(
    ...
    study = optuna.load_study(
        study_name='saddle_approximator',
        storage=storage
    )
    
    best_params = {param: value for param, value in study.best_params.items() if not param.startswith('num_units_layer')}
    ...
    model, best_val_loss = trainer(
        ...
        num_workers=args.num_workers,   
        input_shape=args.input_shape,
        units_per_layer=[study.best_params[f'num_units_layer_{i}'] for i in range(study.best_params['num_layers'])],
        **best_params
    )
